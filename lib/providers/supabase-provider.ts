...
function mapProspectToRow(prospect: Prospect) {
  return {
    ...
    proof_viewed_at: prospect.proofViewedAt || null,
    proof_view_count: prospect.proofViewCount ?? 0,
    email_sent_at: prospect.emailSentAt || null,
    email_opened_at: prospect.emailOpenedAt || null,
    email_clicked_at: prospect.emailClickedAt || null,
    email_replied_at: prospect.emailRepliedAt || null,
    email_bounced_at: prospect.emailBouncedAt || null,
    email_open_count: prospect.emailOpenCount ?? 0,
    email_click_count: prospect.emailClickCount ?? 0,
    tracking_token: prospect.trackingToken || null,
    proof_url: prospect.proofUrl || null,
    proof_screenshot_url: prospect.proofScreenshotUrl || null,
    proof_video_url: prospect.proofVideoUrl || null,
    proof_status: prospect.proofStatus || 'none',
    proof_generated_at: prospect.proofGeneratedAt || null,
...
export async function readSupabaseData(): Promise<SalesOsData> {
  ...
  return {
    ...
    prospects: (prospects.data || []).map((row: any) => ({
      ...
      proofViewedAt: row.proof_viewed_at || undefined,
      proofViewCount: row.proof_view_count ?? 0,
      emailSentAt: row.email_sent_at || undefined,
      emailOpenedAt: row.email_opened_at || undefined,
      emailClickedAt: row.email_clicked_at || undefined,
      emailRepliedAt: row.email_replied_at || undefined,
      emailBouncedAt: row.email_bounced_at || undefined,
      emailOpenCount: row.email_open_count ?? 0,
      emailClickCount: row.email_click_count ?? 0,
      trackingToken: row.tracking_token || undefined,
      proofUrl: row.proof_url || `/proof/${String(row.business_name || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')}` || undefined,
      proofScreenshotUrl: row.proof_screenshot_url || undefined,
      proofVideoUrl: row.proof_video_url || undefined,
      proofStatus: row.proof_status || 'none',
      proofGeneratedAt: row.proof_generated_at || undefined,
