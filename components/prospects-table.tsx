...
import { getProspectEmailEngagement } from "@/lib/email-tracking"
...
export function ProspectsTable({ prospects }: { prospects: Prospect[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    list.sort((a, b) => {
      const engagementDiff = getProspectEmailEngagement(b).sortWeight - getProspectEmailEngagement(a).sortWeight
          ...
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-3 py-2.5 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Email</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((p) => {
                const engagement = getProspectEmailEngagement(p)
                return (
                  <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-2">
                        ...
                        {engagement.hotLead && <Badge className="text-[10px] bg-orange-500 text-white border-0">🔥</Badge>}
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-2.5">
                      <div className="flex flex-col gap-1">
                        <Badge variant="outline" className={`w-fit text-[10px] ${engagement.emailBadgeClass}`}>
                          {engagement.emailLabel}
                        </Badge>
                        <span className="text-[11px] text-slate-400">{engagement.activityLabel || 'No activity yet'}</span>
                      </div>
                    </td>
