...
import { Phone, Globe, Mail, MessageSquare, CheckCircle, ExternalLink } from "lucide-react";
...
export function CallQueue({ prospects }: { prospects: Prospect[] }) {
  ...
  const niches = [...new Set(prospects.map(p => p.vertical || p.niche || "unknown"))];
  const markets = [...new Set(prospects.map(p => p.marketTag || p.market || "unknown"))];
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-5">
        <h2 className="text-lg font-bold text-sales-900 mr-auto">Call Queue</h2>
        <select
      ...
      {filtered.length === 0 && (
        <div className="text-center py-12 text-slate-400">
          ...
          <p className="text-sm mt-1">Try adjusting your vertical or market filter</p>
        </div>
      )}
    </div>
  );
...
function ProspectCard({ prospect }: { prospect: Prospect }) {
  ...
  const hook = prospect.callOpener || prospect.idealPitchAngle || "";
    ...
    <Card className="group hover:shadow-md hover:border-blue-200 transition-all duration-200">
      <CardContent className="pt-5 pb-4 px-5">
        ...
        <div className="space-y-1.5 mb-3 text-sm">
          {prospect.phone && (
            <a href={`tel:${prospect.phone}`} className="flex items-center gap-2 text-slate-600 hover:text-blue-600 transition-colors">
              <Phone className="w-3.5 h-3.5 text-slate-400" />
              <span>{prospect.phone}</span>
            </a>
          )}
          {prospect.website && (
            <a href={prospect.website.startsWith("http") ? prospect.website : `https://${prospect.website}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-slate-600 hover:text-blue-600 transition-colors truncate">
        ...
        <div className="flex items-center gap-2 pt-3 border-t border-slate-100">
          ...
          <Button size="sm" variant="outline" className="text-xs h-8 px-3 rounded-lg border-slate-200" asChild>
            <a href={`sms:${prospect.phone}`}>
              <MessageSquare className="w-3 h-3 mr-1" /> Text
            </a>
          </Button>
          <Button size="sm" variant="outline" className="text-xs h-8 px-3 rounded-lg border-slate-200" asChild>
            <a href={`mailto:?subject=Re: ${prospect.businessName}`}>
              <Mail className="w-3 h-3 mr-1" /> Email
            </a>
          </Button>
          <Button size="sm" variant="outline" className="text-xs h-8 px-3 rounded-lg border-emerald-200 text-emerald-600 hover:bg-emerald-50 ml-auto">
